...
const box = Array.from(document.querySelectorAll('.cell'));
...
// functions for each event on the cells
...
function compareFlipped(array) {
	...
	if (array.length == 2) {
        const cardi=array[0]
        const carde=array[1]
		const card1 = array[0].getAttribute('data-cell-index');
        const card2 = array[1].getAttribute('data-cell-index');
        

		if (card1 == card2 ) {
		 console.log('Yay its a match');
            const c1=cardi.classList.add('matchingcards')
			const c2 = carde.classList.add('matchingcards');
			correct_flips += 1;
			last_flipped = [];
		} else {
			setTimeout(() => {
...
function gameWonParty(moves) {
         document.getElementById('winner').innerHTML=`Hurray !!! you won with ${moves} moves in ${minutes_str}:${seconds_str}`
}
